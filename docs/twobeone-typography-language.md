...
- **Dashboard statistics:** the four activity cards use a number-first display role, requested on 13 September 2026: responsive 32–64px tabular numbers at weight 700, followed by the shared card-title and supporting-copy roles. Use the same metric scale for all four cards; allow the grid to reflow when text is enlarged. This exception does not change titles or numbers elsewhere.
