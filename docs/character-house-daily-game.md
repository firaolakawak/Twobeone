...
The couple-facing house is now a shared goal for Together in Faith. Setup asks only for a house type and bedroom count. **Start building** saves those choices for the couple. Either partner can later change the shared house type and bedroom count without resetting construction progress. Decorating, floor planning, camera controls, and the 3D scene are removed from this flow.
...
- All **487 tests in 97 files** passed. The production build passed with the existing large-chunk advisory.
- **15 isolated PostgreSQL/PGlite scenarios** passed, covering two-person completion, daily deduplication, setup and design updates, saved design/progress preservation, current partner authorization, skipped days, and the 365-block cap. PGlite uses one connection; actual multi-session lock contention was not tested.
...
- The design editor and transparent icon trigger were additionally checked at 320px and 390px in headless Chrome with English, Amharic, Afaan Oromo, and representative 200% root text scaling. Those checks found no page overflow or functional text below 12px.
