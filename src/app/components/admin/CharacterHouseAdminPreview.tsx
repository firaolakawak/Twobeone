...
function AdminCounter({ label, icon: Icon, value, range, onChange }: { label: string; icon: typeof Layers3; value: number; range: [number, number]; onChange: (value: number) => void }) {
  return (
    <label className="block min-w-0 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
      <span className="tbo-label mb-2 flex items-center gap-2 text-slate-600"><Icon className="h-4 w-4 shrink-0 text-violet-600" />{label}</span>
      <input className="h-10 min-w-0 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 tbo-field text-slate-950 outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-200" type="number" min={range[0]} max={range[1]} value={value} onChange={event => onChange(clampToRange(Number(event.target.value) || range[0], range))} />
    </label>
  );
}
  ...
  return (
    <main className="min-w-0 w-full space-y-5 pb-10 [overflow-wrap:anywhere]">
      <header className="overflow-hidden rounded-[2rem] border border-violet-200 bg-gradient-to-br from-slate-950 via-violet-950 to-rose-950 p-6 text-white shadow-xl">
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div className="min-w-0 max-w-2xl">
            <span className="tbo-eyebrow inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-violet-100"><ShieldCheck className="h-4 w-4 shrink-0" />  {tr("Administrator-only preview")}</span>
            <h1 className="mt-4 tbo-page-title">{tr("Character House Game Studio")}</h1>
            <p className="tbo-supporting mt-2 max-w-xl text-violet-100/80">{tr("Inspect the complete 365-day experience, test construction milestones, and compare every architectural configuration without changing a couple’s real progress.")}</p>
          </div>
          <Button type="button" variant="outline" onClick={resetPreview} className="h-auto min-h-10 max-w-full whitespace-normal border-white/20 bg-white/10 text-white hover:bg-white/20 hover:text-white"><RotateCcw className="mr-2 h-4 w-4" /><span className="min-w-0">{tr("Reset final preview")}</span></Button>
        </div>
      </header>
      ...
      <section className="grid gap-4 sm:grid-cols-3" aria-label={tr("Game preview summary")}>
        ...
        <Card className="rounded-2xl border-violet-200 bg-violet-50"><CardContent className="flex items-center gap-3 p-4"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-violet-600 text-white"><Building2 className="h-5 w-5" /></span><div className="min-w-0"><strong className="tbo-section-title text-violet-950">{tr(stage.name)}</strong><p className="tbo-caption text-violet-700">{tr("Current construction stage")}</p></div></CardContent></Card>
        ...
        <aside className="min-w-0 space-y-4">
          <Card className="rounded-[1.5rem] border-slate-200">
            ...
            <CardContent className="space-y-4">
              <label className="block min-w-0"><span className="tbo-label mb-1.5 block text-slate-600">{tr("Home design")}</span><select value={config.homeType} onChange={event => selectHome(event.target.value as HomeType)} className="tbo-field h-11 min-w-0 max-w-full w-full rounded-xl border border-slate-200 bg-white px-3 text-slate-900">{HOME_DEFINITIONS.map(home => <option key={home.id} value={home.id}>{tr(home.name)}</option>)}</select></label>
        ...
        <div className="min-w-0 space-y-4">
          <Card className="overflow-hidden rounded-[2rem] border-slate-200 shadow-lg">
            <CardHeader className="space-y-4 border-b border-slate-100 bg-white">
              <div className="flex flex-wrap items-center justify-between gap-3"><div className="min-w-0"><p className="tbo-eyebrow uppercase text-violet-600">{tr(stage.name)} · <span lang="en">{stage.verse}</span></p><CardTitle className="mt-1">{tr("Day {day} administrator preview", { day })}</CardTitle></div><Button type="button" onClick={() => setDay(365)} className="h-auto min-h-10 max-w-full whitespace-normal rounded-xl bg-violet-700 hover:bg-violet-800"><CheckCircle2 className="mr-2 h-4 w-4" /><span className="min-w-0">{tr("Show final result")}</span></Button></div>
              <div><div className="tbo-label mb-1.5 flex flex-wrap justify-between gap-2 text-slate-600"><span>{tr("Construction timeline")}</span><span>{Math.round(progress)}%</span></div><Progress value={progress} className="h-2.5" /></div>
