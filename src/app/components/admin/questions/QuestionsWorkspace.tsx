...
function previewLanguage(language?: string): 'en' | 'am' | 'om' | undefined {
  return language === 'en' || language === 'am' || language === 'om' ? language : undefined;
}

function PromptPreview({ prompt, index, language }: { prompt: QuestionPrompt; index: number; language?: 'en' | 'am' | 'om' }) {
  return (
    <article className="qa-preview__prompt">
      <div><span>{String(index + 1).padStart(2, '0')}</span><small>{typeNames[prompt.type]}</small></div>
      <h3 lang={language}>{prompt.text}</h3>
      {prompt.type === 'text' && <div className="qa-preview__text-answer">Write your response…</div>}
      {(prompt.type === 'multiple_choice' || prompt.type === 'multiple_select') && <div className="qa-preview__options" lang={language}>{(prompt.options || []).map((option) => <span key={option}><i />{option}</span>)}</div>}
...
export function QuestionsWorkspace(props: Props) {
  ...
  const selectedLanguage = previewLanguage(selectedQuestion?.language);
