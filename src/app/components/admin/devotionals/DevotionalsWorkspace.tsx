...
export function DevotionalsWorkspace(props: Props) {
  ...
  const selectedLanguage = previewLanguage(selectedDevotional?.language);
            ...
            return (
              <article className="dev-console__row" data-active={active || undefined} key={devotional.id}>
                <button className="dev-console__row-main" type="button" onClick={() => onSelect(devotional.id)} aria-label={`Preview ${devotional.title}`}>
                  ...
                  <div><div className="dev-console__row-title"><h2 lang={previewLanguage(devotional.language)}>{devotional.title}</h2><span className={`dev-console__status dev-console__status--${devotional.status}`}>{devotional.status}</span></div><p lang={devotional.reference ? previewLanguage(devotional.language) : undefined}>{devotional.reference || 'Scripture reference pending'}</p><small>{languageNames[(devotional.language ?? 'en') as keyof typeof languageNames] ?? devotional.language}{devotional.audioUrl ? ' · Audio ready' : ''}</small></div>
        ...
        <aside className="dev-console__preview" aria-label="Devotional preview">
          {selectedDevotional ? (
            <>
              ...
              <h2 lang={selectedLanguage}>{selectedDevotional.title}</h2>
              <blockquote lang={selectedLanguage}><p>“{selectedDevotional.verse}”</p><cite>{selectedDevotional.reference}</cite></blockquote>
              <div className="dev-console__preview-section"><span>Reflection</span><p lang={selectedDevotional.reflection ? selectedLanguage : undefined}>{selectedDevotional.reflection || 'Reflection content has not been added.'}</p></div>
              <div className="dev-console__prayer"><span>Prayer prompt</span><p lang={selectedDevotional.prayerPrompt ? selectedLanguage : undefined}>{selectedDevotional.prayerPrompt || 'Prayer prompt has not been added.'}</p></div>
              {!!selectedDevotional.tags?.length && <div className="dev-console__tags" lang={selectedLanguage}>{selectedDevotional.tags.map((tag) => <span key={tag}>#{tag}</span>)}</div>}
