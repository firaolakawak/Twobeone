    ...
    <main className="min-h-screen bg-gradient-to-br from-rose-50 via-white to-amber-50 px-4 py-10">
      <section className="mx-auto w-full max-w-md overflow-hidden rounded-[2rem] border border-rose-100 bg-white shadow-[0_24px_70px_-28px_rgba(136,19,55,.35)]">
        <header className="bg-gradient-to-br from-rose-500 to-pink-600 px-7 py-7 text-white">
          ...
          <h1 className="tbo-page-title text-white">{copy.title}</h1>
        ...
        <div className="p-7">
          {checking ? (
            ...
            <div className="space-y-5 py-4 text-center">
              ...
              <button type="button" onClick={onComplete} className="h-12 w-full rounded-full bg-rose-600 tbo-action text-white hover:bg-rose-700">{copy.continueToSignIn}</button>
            </div>
          ) : (
            <form className="space-y-4" onSubmit={updatePassword}>
              ...
              <label className="grid gap-2 tbo-label">
                ...
                <input type={showPassword ? 'text' : 'password'} value={confirmation} onChange={(event) => setConfirmation(event.target.value)} autoComplete="new-password" disabled={!ready || saving} className="tbo-field h-12 w-full rounded-xl border border-border bg-white px-4 outline-none focus:border-rose-400 focus:ring-4 focus:ring-rose-100" required />
              </label>
              {error && <div role="alert" className="rounded-xl bg-red-50 p-3 tbo-supporting text-red-700">{copy[error]}</div>}
              <button type="submit" disabled={!ready || saving} className="flex h-12 w-full items-center justify-center gap-2 rounded-full bg-rose-600 tbo-action text-white hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-50">
