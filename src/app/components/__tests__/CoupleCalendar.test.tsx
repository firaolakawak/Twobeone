...
describe("CoupleCalendar", () => {
  ...
  it("creates one synchronized calendar item with automatic prayer enabled", async () => {
    const item = {
      ...
      createPrayer: true,
      prayerId: "prayer-1",
      prayerTitle: "Prayer for Marriage retreat",
      prayerText: "Lord, guide us.",
    ...
    const fetchMock = vi.fn(async (_url: string, init?: RequestInit) => {
      if (init?.method === "POST") {
        return {
          ok: true,
          json: async () => ({ item, prayer: { id: "prayer-1" } }),
        };
      }
    ...
    await user.click(
      screen.getAllByRole("button", { name: "Create together" })[0],
    );
    await user.type(screen.getByLabelText("Title"), "Marriage retreat");
    await user.click(
      screen.getByRole("button", { name: "Choose an event emoji: 🎉" }),
    );
    expect(
      screen.getByRole("switch", { name: "Create a prayer automatically" }),
    ).toBeChecked();
    await user.click(
      screen.getByRole("button", { name: "Add to our calendar" }),
    );
  ...
  it("switches between weekly, monthly, and yearly marked-day views", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "December" }),
    ).toBeInTheDocument();
  });
  ...
  it("unifies milestones and recent journals with the calendar page", async () => {
          ...
          journalEntries={[
            {
              id: "journal-1",
              title: "A joyful day",
              content: "We prayed and celebrated.",
    ...
    expect(
      await screen.findByText("Relationship Milestones"),
    ).toBeInTheDocument();
  ...
  it("loads every recorded couple activity into the calendar sync summary", async () => {
    const activities = [
      ["prayer-1", "prayer", "Prayer", "🙏"],
      ["devotional-1", "devotional", "Devotional completed", "📖"],
      ["qa-1", "qa", "Question answered", "💬"],
      ["journal-1", "journal", "Journal reflection", "✍️"],
      ["verse-1", "verse", "John 3:16", "📜"],
      ["mood-1", "mood", "great", "🤩"],
      ["stage-1", "stage", "Couple stage 1", "🌱"],
    ].map(([id, type, title, emoji]) => ({
      id,
      type,
      title,
      emoji,
  ...
  it("upgrades an existing calendar fallback prayer without creating a new link", async () => {
    const legacyItem = {
      id: "cal-legacy",
      userId: "user-1",
      title: "Keti Invite to UAE",
      description: "Prepare the invitation together",
      type: "plan",
      ...
      status: "upcoming",
      createPrayer: true,
      prayerId: "prayer-existing",
      prayerTitle: "Prayer for Keti Invite to UAE",
      prayerText: "Lord, we place our plan in Your hands.",
    ...
    const upgradedItem = {
      ...
      prayerText:
        "Lord, guide every step of Keti’s invitation and unite us in wisdom and peace. Amen.",
    ...
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      ...
      if (
        url.endsWith("/cal-legacy/regenerate-prayer") &&
        init?.method === "POST"
      ) {
        return {
          ok: true,
          json: async () => ({ success: true, item: upgradedItem }),
        };
      }
  ...
  it("keeps linked prayers on the dedicated prayer page instead of duplicating a calendar tab", async () => {
    const item = {
      id: "cal-actions",
      userId: "user-1",
      title: "Church visit",
      description: "Attend together",
      type: "event",
      category: "faith",
      emoji: "⛪",
      startsAt: "2026-09-20T18:00:00.000Z",
      ...
      status: "upcoming",
      createPrayer: true,
      prayerId: "prayer-actions",
      prayerTitle: "Prayer for Church visit",
      prayerText: "Lord, guide our worship. Amen.",
      scripture: "Proverbs 3:5–6",
      prayerGenerationSource: "ai",
      createdAt: "2026-08-18T12:00:00.000Z",
      updatedAt: "2026-08-18T12:00:00.000Z",
    ...
    expect(
      screen.queryByRole("button", { name: "Prayer list" }),
    ).not.toBeInTheDocument();
    expect(screen.getByText("Prayer-linked")).toBeInTheDocument();
  });
});
