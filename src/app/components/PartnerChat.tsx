...
export function PartnerChat({ accessToken, currentUserId, partnerName = 'Partner', partnerOnline = false, onBack, onUnreadChange }: PartnerChatProps) {
  ...
  return (
    <section onPointerDown={primeMessageSound} onKeyDown={primeMessageSound} className="mx-auto flex min-h-[calc(100dvh-10rem)] w-full max-w-2xl flex-col overflow-hidden rounded-[2rem] border border-rose-100 bg-card shadow-[0_24px_70px_-48px_rgba(190,24,93,.6)]" aria-label={text.title}>
      <header className="tbo-section-hero relative overflow-hidden border-b border-rose-100 px-5 py-5 text-foreground">
        <div className="relative flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1"><h1 className="tbo-h2 break-words">{partnerName}</h1><p className="tbo-caption text-muted-foreground">{partnerOnline ? text.online : text.offline}</p></div>
          <span className="hidden items-center gap-1.5 rounded-full bg-white/15 px-3 py-2 tbo-caption sm:flex"><Lock className="h-3 w-3" />{text.private}</span>
        </div>
      </header>

      {!hasPartner ? (
        <div className="flex flex-1 flex-col items-center justify-center px-8 py-16 text-center"><span className="grid h-20 w-20 place-items-center rounded-full bg-rose-50 text-rose-500"><UserPlus className="h-10 w-10" /></span><h2 className="mt-5 tbo-h2 text-slate-900">{text.connect}</h2><p className="mt-2 max-w-sm text-sm leading-6 text-slate-500">{text.connectHint}</p></div>
      ) : (
        <>
          <div className="flex-1 overflow-y-auto bg-background px-4 py-5 sm:px-6">
            {loading ? <div className="grid h-full place-items-center"><Loader2 className="h-7 w-7 animate-spin text-rose-500" /></div> : messages.length === 0 ? (
              <div className="flex h-full min-h-80 flex-col items-center justify-center text-center"><span className="grid h-20 w-20 place-items-center rounded-full bg-rose-100 text-rose-600"><MessageCircleHeart className="h-10 w-10" /></span><h2 className="mt-5 tbo-h2 text-slate-900">{text.empty}</h2><p className="mt-2 max-w-sm text-sm leading-6 text-slate-500">{text.emptyHint}</p></div>
            ) : Object.entries(grouped).map(([key, dayMessages]) => (
              <div key={key} className="mb-6"><div className="mb-4 flex items-center gap-3"><span className="h-px flex-1 bg-rose-100" /><span className="tbo-caption text-rose-500">{formatDay(dayMessages[0].createdAt)}</span><span className="h-px flex-1 bg-rose-100" /></div><div className="space-y-3">{dayMessages.map(message => {
                const own = message.senderId === currentUserId;
                return <div key={message.id} className={`flex ${own ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[82%] sm:max-w-[70%]`}><div className={`rounded-2xl px-4 py-3 text-sm leading-6 shadow-sm ${own ? 'rounded-br-md bg-rose-600 text-white' : 'rounded-bl-md bg-card text-slate-800 ring-1 ring-slate-100'}`}><p className="tbo-body-l whitespace-pre-wrap [overflow-wrap:anywhere]">{message.message}</p></div><p className={`mt-1 flex items-center gap-1 px-1 tbo-caption text-slate-500 ${own ? 'justify-end' : 'justify-start'}`}>{new Intl.DateTimeFormat(undefined, { hour: 'numeric', minute: '2-digit' }).format(new Date(message.createdAt))}{own && <CheckCheck className="h-3 w-3 text-rose-400" />}</p></div></div>;
              })}</div></div>
            ))}
            <div ref={endRef} />
          </div>
          <form onSubmit={event => { event.preventDefault(); void sendMessage(); }} className="border-t border-rose-100 bg-card p-3 sm:p-4">
            <div className="flex items-end gap-2 rounded-2xl border border-slate-200 bg-background p-2 focus-within:border-rose-300 focus-within:ring-4 focus-within:ring-rose-50"><textarea value={draft} onChange={event => setDraft(event.target.value.slice(0, 2000))} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void sendMessage(); } }} rows={2} maxLength={2000} placeholder={text.placeholder} aria-label={text.placeholder} className="field-sizing-content max-h-32 min-h-[68px] min-w-0 flex-1 resize-none bg-transparent px-3 py-2.5 text-base leading-6 text-slate-900 outline-none placeholder:text-slate-400" /><Button type="submit" size="icon" disabled={!draft.trim() || sending} className="h-11 w-11 shrink-0 rounded-xl bg-rose-600 text-white hover:bg-rose-700" aria-label={text.send}>{sending ? <Loader2 className="h-5 w-5 animate-spin" /> : <Send className="h-5 w-5" />}</Button></div>
            <p className="mt-2 flex items-center justify-center gap-1 tbo-caption text-slate-500"><Heart className="h-3 w-3 fill-rose-300 text-rose-300" />{text.private}</p>
          </form>
        </>
      )}
    </section>
