...
export function PrayerBoard({
  ...
  const [commentPreviews, setCommentPreviews] = useState<Record<string, PrayerComment | null>>({});
  const [commentFocusTarget, setCommentFocusTarget] = useState<string | null>(null);
  const [commentControlFocusTarget, setCommentControlFocusTarget] = useState<string | null>(null);

  useEffect(() => {
    setCommentPreviews((current) => {
      let changed = false;
      const next = { ...current };
      const visiblePrayerIds = new Set(prayers.map((prayer) => prayer.id));

      Object.keys(next).forEach((prayerId) => {
        if (!visiblePrayerIds.has(prayerId)) {
      ...
      prayers.forEach((prayer) => {
        if (prayer.latestComment === undefined) return;
        const existing = current[prayer.id];
        const incoming = prayer.latestComment;
        if (existing?.id !== incoming?.id || (existing === undefined && incoming === null)) {
          next[prayer.id] = incoming;
          changed = true;
        }
      });
  ...
  useEffect(() => {
    if (!commentFocusTarget || !openCommentThreads.has(commentFocusTarget)) return;
    const commentRegion = document.getElementById(`prayer-comments-${commentFocusTarget}`);
    if (!commentRegion) return;
    commentRegion.focus();
    setCommentFocusTarget(null);
  }, [commentFocusTarget, openCommentThreads]);

  useEffect(() => {
    if (!commentControlFocusTarget || openCommentThreads.has(commentControlFocusTarget)) return;
    const commentControl = document.getElementById(`prayer-comments-toggle-${commentControlFocusTarget}`);
    if (!commentControl) return;
    commentControl.focus();
    setCommentControlFocusTarget(null);
  }, [commentControlFocusTarget, openCommentThreads]);

  const handleLatestCommentChange = useCallback((prayerId: string, comment: PrayerComment | null) => {
    setCommentPreviews((current) => current[prayerId]?.id === comment?.id
      && !(current[prayerId] === undefined && comment === null)
      ? current
      : { ...current, [prayerId]: comment });
  }, []);
  ...
  const toggleComments = (id: string) => {
    const isOpening = !openCommentThreads.has(id);
    ...
    if (isOpening) setCommentFocusTarget(id);
    else setCommentControlFocusTarget(id);
  };

  const openCommentsFromPreview = (id: string) => {
    setOpenCommentThreads((current) => {
      const next = new Set(current);
      next.add(id);
      return next;
    });
    setCommentFocusTarget(id);
  };
        ...
        <div className="relative">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="tbo-caption inline-flex max-w-full flex-wrap items-center gap-2 rounded-full bg-[var(--glass-inset-surface)] px-3 py-1.5 text-[var(--glass-accent)] shadow-sm ring-1 ring-[var(--glass-rim)]">
                <Heart className="h-3.5 w-3.5 shrink-0 fill-rose-500 text-rose-500" aria-hidden="true" />
                <span className="min-w-0 break-words">{tr("Held together in prayer")}</span>
              </div>
            </div>
            <Button
              type="button"
              variant="outline"
              size="icon"
              onClick={openPrayerForm}
              aria-label={t.prayer.newRequest}
              title={t.prayer.newRequest}
              className="tbo-action h-14 w-14 shrink-0 rounded-full border-2 border-[var(--glass-rose)] bg-[var(--glass-inset-surface)] p-0 text-[var(--glass-rose)] shadow-sm hover:bg-[var(--glass-inset-surface)] hover:text-[var(--glass-rose)]"
            >
              <Plus className="h-8 w-8" aria-hidden="true" />
            </Button>
          </div>
          <h1 className="tbo-page-title mt-5 break-words text-foreground">{t.prayer.title}</h1>
          ...
          <div className="tbo-feature-stats mt-7 grid grid-cols-3 gap-3 border-t border-[var(--glass-border)] pt-5 [overflow-wrap:anywhere]" role="group" aria-label={tr("Prayer sections")}>
            <button
              type="button"
              aria-pressed={activeTab === "requests"}
              onClick={() => setActiveTab("requests")}
              className={`min-h-11 min-w-0 rounded-xl px-1 py-1 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--glass-accent)] ${activeTab === "requests" ? "text-[var(--glass-accent)]" : "text-foreground hover:bg-[var(--glass-inset-surface)]"}`}
            >
              <span className="block text-xl font-bold">{activePrayerCount}</span>
              <span className={`tbo-caption mt-0.5 block ${activeTab === "requests" ? "text-[var(--glass-accent)]" : "text-muted-foreground"}`}>{tr("Active")}</span>
            </button>
            <button
              type="button"
              aria-pressed={activeTab === "together"}
              onClick={() => setActiveTab("together")}
              className={`min-h-11 min-w-0 rounded-xl border-l border-[var(--glass-border)] px-3 py-1 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--glass-accent)] ${activeTab === "together" ? "text-[var(--glass-accent)]" : "text-foreground hover:bg-[var(--glass-inset-surface)]"}`}
            >
              <span className="block text-xl font-bold">{togetherPrayerCount}</span>
              <span className={`tbo-caption mt-0.5 block ${activeTab === "together" ? "text-[var(--glass-accent)]" : "text-muted-foreground"}`}>{tr("Together")}</span>
            </button>
            <button
              type="button"
              aria-pressed={activeTab === "answered"}
              onClick={() => setActiveTab("answered")}
              className={`min-h-11 min-w-0 rounded-xl border-l border-[var(--glass-border)] px-3 py-1 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--glass-accent)] ${activeTab === "answered" ? "text-[var(--glass-accent)]" : "text-foreground hover:bg-[var(--glass-inset-surface)]"}`}
            >
              <span className="block text-xl font-bold">{answeredPrayerCount}</span>
              <span className={`tbo-caption mt-0.5 block ${activeTab === "answered" ? "text-[var(--glass-accent)]" : "text-muted-foreground"}`}>{tr("Answered")}</span>
            </button>
          </div>
        </div>
      </header>
      ...
      <div className="space-y-4">
        {!hasPartner && activeTab === "together" ? (
          ...
          filteredPrayers.map((prayer) => {
            const category = getCategoryData(prayer.category);
            const requesterFirstName = (prayer.isPartner ? partnerName : userName)
              ?.trim()
              .split(/\s+/u)[0];
            const canTrack = !prayer.isCommunity && !prayer.isLockedForPartner;
            const canManage = !prayer.isPartner && !prayer.isCommunity;
            const optionsOpen = expandedCards.has(prayer.id);
            const commentsOpen = openCommentThreads.has(prayer.id);
            const commentsRegionId = `prayer-comments-${prayer.id}`;
            const currentUserPrayed = Boolean(prayer.isPartner ? prayer.partnerPrayed : prayer.youPrayed);
            const latestComment = Object.prototype.hasOwnProperty.call(commentPreviews, prayer.id)
              ? commentPreviews[prayer.id]
              : prayer.latestComment;
            const prayerContent = prayer.isLockedForPartner
              ? tr('Surprise — locked until {date}.', { date: prayer.unlockAt ? formatDate(prayer.unlockAt) : tr('the reveal date') })
              : prayer.description;
            const visiblePrayerContent = optionsOpen || prayer.isLockedForPartner
              ? prayerContent
              : getHalfTextPreview(prayerContent);
            const prayerActions = (
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => handleTogglePrayed(prayer)}
                  aria-label={tr("You prayed")}
                  title={tr("You prayed")}
                  aria-pressed={currentUserPrayed}
                  className={`tbo-action flex h-12 w-12 items-center justify-center rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--glass-accent)] ${currentUserPrayed ? "bg-rose-50 text-rose-600" : "text-muted-foreground hover:bg-[var(--glass-inset-surface)] hover:text-[var(--glass-accent)]"}`}
                >
                  <span className={`flex h-6 w-6 items-center justify-center rounded-full ${currentUserPrayed ? "bg-rose-500 text-white" : "border-2 border-current"}`}>
                    <Check className="h-4 w-4" aria-hidden="true" />
                  </span>
                </button>
                <button
                  id={`prayer-comments-toggle-${prayer.id}`}
                  type="button"
                  ...
                  className={`tbo-action flex h-12 w-12 items-center justify-center rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--glass-accent)] ${commentsOpen ? "bg-[var(--glass-inset-surface)] text-[var(--glass-accent)]" : "text-muted-foreground hover:bg-[var(--glass-inset-surface)] hover:text-[var(--glass-accent)]"}`}
                >
              ...
              >
                <CardContent className="p-0">
                  <div className="tbo-feature-card-content p-5 sm:p-6">
                    <div className="tbo-feature-card-heading flex min-w-0 items-start gap-4">
                      <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-full bg-[var(--glass-inset-surface)] text-xl ring-1 ring-[var(--glass-rim)]" aria-hidden="true">
                        {prayer.isLockedForPartner
                          ? <Lock className="h-5 w-5 text-[var(--glass-accent)]" />
                          : category.icon}
                      </div>

                      <div className="min-w-0 flex-1">
                        <div className="tbo-caption flex flex-wrap items-center gap-x-2 gap-y-1 text-muted-foreground">
                          {requesterFirstName && (
                            <>
                              <span className="font-semibold text-foreground">{requesterFirstName}</span>
                          ...
                          {prayer.isLockedForPartner ? (
                            <Badge variant="outline" className="h-auto min-h-6 max-w-full whitespace-normal border-[var(--glass-border)] bg-[var(--glass-inset-surface)] px-2 py-0.5 tbo-caption text-muted-foreground">
                              <Lock className="mr-1 h-3 w-3" aria-hidden="true" />
                              {tr("Locked")}
                            </Badge>
                          ) : !prayer.isPartner && prayer.isSurprise ? (
                            <Badge variant="outline" className="h-auto min-h-6 max-w-full whitespace-normal border-[var(--glass-border)] bg-[var(--glass-inset-surface)] px-2 py-0.5 tbo-caption text-muted-foreground">
                              <Lock className="mr-1 h-3 w-3" aria-hidden="true" />
                              {tr("Surprise")}
                            </Badge>
                        ...
                        <h3 className="tbo-card-title mt-2 break-words text-foreground">
                          {prayer.title}
                        </h3>
                        <p className="tbo-body mt-2 whitespace-pre-wrap break-words text-muted-foreground">
                          {visiblePrayerContent}
                        </p>
                      </div>
                      ...
                      {!prayer.isLockedForPartner && (
                        <button
                          ...
                          className={`tbo-action flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--glass-accent)] ${optionsOpen ? "bg-[var(--glass-inset-surface)] text-[var(--glass-accent)]" : "text-muted-foreground hover:bg-[var(--glass-inset-surface)] hover:text-foreground"}`}
                          aria-label={tr(optionsOpen ? 'Collapse {title}' : 'Expand {title}', { title: prayer.title })}
                    ...
                    {canTrack && (
                      <div className="mt-5 border-t border-[var(--glass-border)] pt-2">
                        <PrayerComments
                          id={commentsRegionId}
                          prayerId={prayer.id}
                          prayerTitle={prayer.title}
                          isOpen={commentsOpen}
                          isContentExpanded={optionsOpen}
                          latestComment={latestComment}
                          onOpen={() => openCommentsFromPreview(prayer.id)}
                          onLoadComments={onLoadComments}
                          onLoadLatestComment={onLoadLatestComment}
                          onAddComment={onAddComment}
                          onLatestCommentChange={handleLatestCommentChange}
