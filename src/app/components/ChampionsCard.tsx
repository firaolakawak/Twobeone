      ...
      <div className="relative">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="tbo-caption text-amber-700">{labels.title}</p>
            <h3 className="tbo-h3 text-slate-900">{levelLabel}</h3>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          {([['today', summary.today], ['week', summary.week], ['month', summary.month]] as const).map(([key, period]) => (
            <div key={key} className="rounded-2xl border border-white/80 bg-white/75 px-2 py-3 text-center shadow-sm">
              <p className="tbo-caption text-slate-500">{labels[key]}</p>
          ...
          <>
            <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3">
              {categories.map(category => {
                ...
                return <div key={category} className="flex items-center gap-2"><Icon className="h-4 w-4 text-rose-500" /><span className="min-w-0 flex-1 break-words text-xs text-slate-600">{labels[category]}</span><strong className="text-xs tabular-nums text-slate-800">{formatEngagementTime(summary.week.byCategory[category], language)}</strong></div>;
              })}
            </div>
            ...
            {summary.champion.nextTargetSeconds && <p className="mt-1.5 text-right tbo-caption text-slate-500">{summary.champion.progress}% {labels.next}</p>}
          </>
        )}
      </div>
    </section>
