-- Store prayer-request comments in the existing prayer chat partition while
-- keeping them in a collision-free key namespace.
...
as $$
  select case
    when p_key like 'webrtc_%' then 'realtime_state'
      ...
      when 'prayer-comment' then 'prayer_chats'
