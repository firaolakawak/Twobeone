...
export function registerCharacterHouseRoutes(app: any, dependencies: HouseDependencies) {
  for (const action of ['get', 'start', 'update'] as const) {
    app[action === 'get' ? 'get' : 'post'](`/make-server-6d579fee/character-house${action === 'get' ? '' : `/${action}`}`, async (c: any) => {
      ...
      try {
        ...
        if (data.code === 'house_not_started') return c.json({ error: 'Start your shared house before changing its design.', code: data.code }, 409);
